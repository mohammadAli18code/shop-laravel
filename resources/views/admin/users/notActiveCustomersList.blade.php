    ...
    <div class="main-content font-size-13">
        <div class="tab__box">
            <div class="tab__items">
                ...
                <a class="tab__item is-active" href="{{ route('admin.users/not-active-customers') }}">کاربران تاییده
                    نشده</a>
                ...
                <tbody>
                    @foreach ($notActiveCustomers as $notActiveCustomer)
                        <tr role="row" class="">
                            ...
                            <td>1.1.1.1</td>
                            @if ($notActiveCustomer->is_active == 1)
                                <td class="text-error">تایید نشده</td>
                            @elseif($notActiveCustomer->is_active == 2)
                                <td class="text-success">تایید شده</td>
                                < @endif
                                    <td>
                                        <a href="{{ route('admin/customers/delete/' . $notActiveCustomer->id) }}"
                                            class="item-delete mlg-15" title="حذف"></a>
                                        <a href="{{ route('admin/customers/confirm/' . $notActiveCustomer->id) }}"
                                            class="item-confirm mlg-15" title="تایید"></a>
                                        <a href="{{ route('admin/customers/not-confirm/' . $notActiveCustomer->id) }}"
                                            class="item-reject mlg-15" title="رد"></a>
                                        <a href="edit-user.html" target="_blank" class="item-eye mlg-15" title="مشاهده"></a>
                                        <a href="{{ route('admin/profile/customer/edit/' . $notActiveCustomer->id) }}"
